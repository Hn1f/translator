...
exports.IdentifierScanner = void 0;
 ...
 */
function roleFromSemanticTokenType(tokenType) {
    switch (tokenType) {
        case 'class':
        case 'interface':
        case 'struct':
 ...
 */
function inferRoleHeuristic(line, matchStart, text) {
...
class IdentifierScanner {
     ...
     */
    static async scanViaSemanticTokens(document, range) {
        try {
     ...
     */
    static findExcludedRanges(line) {
        const ranges = [];
        let i = 0;
        let quote = null;
        let stringStart = -1;
        ...
        if (quote)
            ranges.push([stringStart, line.length]);
        return ranges;
    }
    static isExcluded(index, ranges) {
        return ranges.some(([start, end]) => index >= start && index < end);
    }
    /** Fallback path for languages without a semantic tokens provider. */
    static scanViaRegex(document, range) {
        const results = [];
        for (let lineNo = range.start.line; lineNo <= range.end.line; lineNo++) {
            ...
            for (const match of line.matchAll(IDENTIFIER_RE)) {
                const text = match[0];
                if (KEYWORD_DENYLIST.has(text))
                    continue;
                const startChar = match.index ?? 0;
                if (this.isExcluded(startChar, excluded))
                    continue; // inside a string literal or // comment
                const script = detectScript(text);
                if (script === 'en' || script === 'unknown')
                    continue;
                results.push({
    ...
    static async scan(document, range) {
        const viaSemantic = await this.scanViaSemanticTokens(document, range);
        return viaSemantic ?? this.scanViaRegex(document, range);
    }
}
exports.IdentifierScanner = IdentifierScanner;
//# sourceMappingURL=IdentifierScanner.js.map