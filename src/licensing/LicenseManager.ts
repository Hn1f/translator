...
type UsageType = 'personal' | 'professional';
...
export class LicenseManager {
  ...
  async runStartupCheck(): Promise<void> {
    try {
      ...
      if (usageType !== 'professional') return;
      ...
      Logger.warn(`License check failed non-fatally: ${err instanceof Error ? err.message : String(err)}`);
    }
  }

  getUsageType(): UsageType | undefined {
    return this.context.globalState.get<UsageType>(STATE_KEY_USAGE_TYPE);
  }

  private async maybeAskUsageType(): Promise<void> {
    if (this.getUsageType() !== undefined) return;

    const choice = await vscode.window.showInformationMessage(
      'Quick question: are you using AI Identifier Translator for personal or professional/commercial work? (Free either way — this just helps with an optional Pro license reminder.)',
      'Personal',
      'Professional',
      "Don't ask again"
    );
  ...
  private shouldNag(): boolean {
    const last = this.context.globalState.get<number>(STATE_KEY_LAST_NAG, 0);
    return Date.now() - last > NAG_INTERVAL_MS;
  }

  private async showProNag(): Promise<void> {
    await this.context.globalState.update(STATE_KEY_LAST_NAG, Date.now());
    const choice = await vscode.window.showInformationMessage(
      "You're using AI Identifier Translator professionally — if it's useful to your team, consider grabbing a Pro license to support development.",
      'Get a License',
  ...
  async promptForLicenseKey(): Promise<void> {
  ...
  async isLicensed(): Promise<boolean> {
    ...
    const lastValidated = this.context.globalState.get<number>(STATE_KEY_LICENSE_VALID, 0);
    if (Date.now() - lastValidated < REVALIDATE_INTERVAL_MS) return true; // trust recent validation, avoid hammering Gumroad
  ...
  private async verifyWithGumroad(licenseKey: string): Promise<boolean> {
    if (GUMROAD_PRODUCT_PERMALINK.startsWith('REPLACE_WITH_')) {
      Logger.warn('Gumroad product permalink not configured — cannot verify license keys yet.');
      return false;
    }
    try {
      ...
      const data = (await res.json()) as { success?: boolean; purchase?: { refunded?: boolean; chargebacked?: boolean } };
      if (!data.success) return false;
      if (data.purchase?.refunded || data.purchase?.chargebacked) return false;
      return true;
    } catch (err) {
      Logger.warn(`Gumroad license verification request failed: ${err instanceof Error ? err.message : String(err)}`);
