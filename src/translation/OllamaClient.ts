...
import { IdentifierRole, TranslationRequestBatch } from '../types';
import { isValidTranslation } from './validation';
...
export class OllamaTranslationError extends Error {}
...
export class OllamaClient {
  ...
  updateSettings(endpoint: string, model: string, temperature: number): void {
   ...
   */
  async translateBatch(batch: TranslationRequestBatch): Promise<Map<string, string>> {
    ...
    const retryRaw = await this.callOllama(
      prompt +
    ...
    throw new OllamaTranslationError(
      `Model returned an unusable response twice for ${batch.filePath} (expected a ${batch.identifiers.length}-element JSON array)`
    );
  }

  private buildPrompt(batch: TranslationRequestBatch): string {
  ...
  private async callOllama(prompt: string): Promise<string> {
    ...
    if (!res.ok) {
      throw new OllamaTranslationError(
        `Ollama request failed: ${res.status} ${res.statusText}. Is Ollama running locally on ${this.endpoint}?`
      );
    }
   ...
   */
  private tryParse(raw: string, batch: TranslationRequestBatch): Map<string, string> | null {
 ...
 * distrust it.
 */
export function transliterationFallback(original: string, role: IdentifierRole): string {
  ...
  if (role === 'class') return slug[0].toUpperCase() + slug.slice(1);
  if (role === 'constant') return slug.toUpperCase();
  return slug;
}
